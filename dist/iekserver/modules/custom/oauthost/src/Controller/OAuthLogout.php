...
class OAuthLogout extends ControllerBase
{
    ...
    public function logoutGo(Request $request)
    {
        ...
        $iekUsers = $this->entityTypeManager->getStorage('iek_users')->loadByProperties(array('authtoken' => $username));
...
//                if ($res->getStatusCode() === "200")
                ...
                $user->save();
                $iekUser->set('accesstoken', '-');
                $iekUser->set('accesstoken_secret', '-');
                $iekUser->set('authtoken','-');
                $iekUser->set('requesttoken','-');
                $iekUser->set('requesttoken_secret', '-');
                $iekUser->save();
